...
export default function Suggestions() {
 ...
 return (
    <div className='friends-section'>
        ...
        {users.length===0 ?<p>No friend suggestions</p>:
            <div className="users-list">{
                    users.map(user=>(<div className="user-card" key={user.id}>
                            <div className="user-info">
                                <div className="user-avatar">{user.name?.charAt(0).toUpperCase()}</div>
                                <div className="user-details"><h4>{user.name}</h4><p>{user.mutual_friends} mutual friends</p></div>
                            </div>
                            <div className="friend-actions">
                                <button className="btn btn-primary" onClick={()=>sendRequest(user.id)}>Add Friend</button>
                            </div>
                        </div>
                    ))}
            </div>}
