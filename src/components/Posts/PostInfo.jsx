...
export default function PostInfo() {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        setPost(set.data);
        setComments({
          items: set.data.comments || [],
          total: set.data.comments_total ?? (set.data.comments?.length || 0),
        });
      } catch (err) {
  ...
  const liked = !!(
    user && post?.likes?.users?.some((person) => person.id === user.id)
  );
  ...
  const addcomment = async () => {
    ...
    try {
      const set = await api.post(`/posts/${id}/comments`, {
        content: newcomment,
      });
      const newc = {
        id: set.data.id,
  ...
  const loadmorecomments = async () => {
    ...
    try {
      const set = await api.get(`/posts/${id}/comments`, {
        params: { skip: comments.items.length, limit: 5 },
      });
      setComments((prev) => {
      ...
      <div className="post-details">
        ...
        <div className="profile-posts">
          <div className="likes-section">
            <p className="likes-count" onClick={togglelikes}>
            ...
            {comments.items.length < comments.total && (
              <div className="see-more-container">
                ...
                >
                  {commentLoading
                    ? "Loading..."
                    : `See more comments (${comments.total - comments.items.length})`}
                </button>
              </div>
            )}
