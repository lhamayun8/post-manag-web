...
export default function PostList() {
    ...
    setCommentsState((prev) => {
      ...
      postList.forEach((p) => {
        if (!next[p.id]) {
          next[p.id] = {
            items: p.comments || [],
            total: p.comments_total ?? (p.comments?.length || 0),
          };
        }
      });
      return next;
  ...
  const fetchPost = async (reset = true, forcerefresh = false) => {
    ...
    try {
      ...
      const set = await api.get("/feed/trending", {
        params: {
  ...
  const addcomment = async (postid) => {
    ...
    try {
      const set = await api.post(`/posts/${postid}/comments`, {
      ...
      setCommentsState((prev) => {
        ...
        return {
          ...prev,
          [postid]: { items: [newc, ...cur.items], total: cur.total + 1 },
        };
      });
  ...
  const loadmorecomments = async (postid) => {
    ...
    try {
      const set = await api.get(`/posts/${postid}/comments`, {
        params: { skip: cur.items.length, limit: 5 },
      });
      setCommentsState((prev) => {
        ...
        {loading && posts.length === 0 ? (
          ...
          posts.map((post) => {
            const comments = commentsState[post.id] || {
              items: post.comments || [],
              total: post.comments_total ?? (post.comments?.length || 0),
            };
            return (
